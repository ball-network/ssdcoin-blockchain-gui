// import { useGetNFTInfoQuery } from '@ssdcoin-network/api-react';
import { fromBech32m } from '@ssdcoin-network/api';
import { AddressBookContext, Color, EmojiAndColorPicker, Flex, Form, TextField } from '@ssdcoin-network/core';
    ...
    filteredAddresses.forEach((entry) => {
      try {
        if (entry.address[3] === '1') {
          if (entry.address.slice(0, 3).toLowerCase() !== 'ssd') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
        } else if (entry.address[4] === '1') {
          if (entry.address.slice(0, 4).toLowerCase() !== 'tssd') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
