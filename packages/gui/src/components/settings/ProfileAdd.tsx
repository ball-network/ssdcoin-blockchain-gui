...
export default function ProfileAdd() {
  ...
  const currencyCode = (useCurrencyCode() ?? 'SSD').toUpperCase();
  const isTestnet = currencyCode === 'TSSD';
  ...
  const spendableBalance = mojoToSSDCoinLocaleString(balance?.spendableBalance);
  const canCreateProfile = (balance?.spendableBalance ?? 0) > 0;

  function handleClick() {
    const url = `https://${isTestnet ? 'testnet10-faucet.ssdcoin.top' : 'faucet.ssdcoin.top'}/?address=${currentAddress}`;
    openExternal(url);
  }
              ...
              {!canCreateProfile && (
                <Flex flexDirection="column" gap={1}>
                  ...
                  <Link onClick={handleClick}>
                    <Trans>Get Mojos from the SSDCoin Faucet</Trans>
                  </Link>
                </Flex>
              )}
              <Flex flexDirection="column" gap={1}>
