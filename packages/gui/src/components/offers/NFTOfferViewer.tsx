import { OfferSummaryRecord, OfferTradeRecord } from '@ssdcoin-network/api';
import { useCheckOfferValidityMutation, useGetWalletsQuery } from '@ssdcoin-network/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['ssd', 'tssd'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.SSDCOIN: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const ssdMakerFee = mojoToSSDCoin(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(ssdMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.SSDCOIN
        ? ssdcoinToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
