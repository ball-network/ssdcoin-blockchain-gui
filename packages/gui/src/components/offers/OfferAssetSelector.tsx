import type { Wallet, CATToken } from '@ssdcoin-network/api';
import { WalletType } from '@ssdcoin-network/api';
import { useGetCatListQuery, useGetWalletsQuery } from '@ssdcoin-network/api-react';
import { Select, useCurrencyCode } from '@ssdcoin-network/core';
...
type BuildAssetSelectorListParams = {
  ...
  ssdcoinCurrencyCode: string;
};

function buildAssetSelectorList(params: BuildAssetSelectorListParams): WalletOfferAssetSelection[] {
  const { wallets, catList, rows, otherRows, selectedWalletId, ssdcoinCurrencyCode } = params;
...
function OfferAssetSelector(props: OfferAssetSelectorProps) {
  ...
  const options: WalletOfferAssetSelection[] = useMemo(() => {
    ...
    return buildAssetSelectorList({
      ...
      otherRows,
      selectedWalletId,
      ssdcoinCurrencyCode: currencyCode,
    });
