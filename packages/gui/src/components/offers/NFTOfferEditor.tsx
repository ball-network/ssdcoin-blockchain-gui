...
import { useCreateOfferForIdsMutation, useGetWalletBalanceQuery } from '@ssdcoin-network/api-react';
import {
  Amount,
  AmountProps,
  Back,
...
/*       An NFT offer consists of a single NFT being offered for SSD/CAT      */
/* ========================================================================== */
          ...
          <Box style={{ position: 'relative', top: '20px' }}>
            <TooltipIcon>
              <Trans>
                ...
                recommended minimum fee is 0.000005 SSD (5,000,000 mojos)
              </Trans>
            </TooltipIcon>
          </Box>
        </Flex>
    ...
    if (
      ...
      switch (tokenWalletInfo.walletType) {
        case WalletType.STANDARD_WALLET:
          balanceString = mojoToSSDCoinLocaleString(walletBalance.spendableBalance, locale);
          balance = mojoToSSDCoin(walletBalance.spendableBalance);
          break;
        case WalletType.CAT:
...
/* ========================================================================== */
/*                              NFT Offer Editor                              */
/*           Currently only supports a single NFT <--> SSD/CAT offer          */
/* ========================================================================== */
...
function buildOfferRequest(params: NFTBuildOfferRequestParams) {
  ...
  const baseMojoAmount: BigNumber =
    tokenWalletInfo.walletType === WalletType.CAT ? catToMojo(tokenAmount) : ssdcoinToMojo(tokenAmount);
  const mojoAmount = exchangeType === NFTOfferExchangeType.NFTForToken ? baseMojoAmount : baseMojoAmount.negated();
  const feeMojoAmount = ssdcoinToMojo(fee);
