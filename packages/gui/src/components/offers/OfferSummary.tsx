...
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToSSDCoin, mojoToCAT } from '@ssdcoin-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['ssd', 'tssd'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToSSDCoin(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['ssd', 'tssd'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToSSDCoin(takerEntries[0][1])
