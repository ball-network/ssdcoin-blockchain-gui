...
import type { OfferSummaryAssetInfo, OfferSummaryRecord } from '@ssdcoin-network/api';
import { mojoToCAT, mojoToSSDCoin, mojoToCATLocaleString, mojoToSSDCoinLocaleString } from '@ssdcoin-network/core';
...
export function summaryStringsForNFTOffer(
  ...
  // TODO: Remove 1:1 NFT <--> SSD assumption
...
export function formatAmountForWalletType(amount: string | number, walletType: WalletType, locale?: string): string {
  if (walletType === WalletType.STANDARD_WALLET) {
    return mojoToSSDCoinLocaleString(amount, locale);
  }
...
export function offerAssetTypeForAssetId(assetId: string, offerSummary: OfferSummaryRecord): OfferAsset | undefined {
  ...
  if (['ssd', 'tssd'].includes(assetId)) {
    assetType = OfferAsset.SSDCOIN;
  } else {
    const { infos } = offerSummary;
    const info: OfferSummaryAssetInfo = infos[assetId];
...
export function offerAssetIdForAssetType(
  ...
  if (assetType === OfferAsset.SSDCOIN) {
    return keys.includes('ssd') ? 'ssd' : undefined;
  }
...
export function getNFTPriceWithoutRoyalties(
  ...
  for (const assetType of [OfferAsset.TOKEN, OfferAsset.SSDCOIN]) {
    const assetId = offerAssetIdForAssetType(assetType, summary);
    if (assetId) {
      const amountInMojos = offerAssetAmountForAssetId(assetId, summary);
      if (amountInMojos) {
        const amountInTokens = assetType === OfferAsset.SSDCOIN ? mojoToSSDCoin(amountInMojos) : mojoToCAT(amountInMojos);
