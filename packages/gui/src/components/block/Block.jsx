import { toBech32m } from '@ssdcoin-network/api';
import { useGetBlockQuery, useGetBlockRecordQuery } from '@ssdcoin-network/api-react';
import {
  Back,
  Button,
  Card,
...
export default function Block() {
  ...
  const poolReward = mojoToSSDCoin(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToSSDCoin(calculateBaseFarmerReward(blockRecord.height));

  const ssdcoinFees = blockRecord.fees !== undefined ? mojoToSSDCoin(blockRecord.fees) : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: ssdcoinFees ? `${ssdcoinFees} ${currencyCode}` : '',
  ...
  return (
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>Block at height {blockRecord.height} in the SSDCoin blockchain</Trans>
          </Back>
        }
        action={
          <Flex gap={1}>
