import { toBech32m, toSnakeCase } from '@ssdcoin-network/api';
import { useSignMessageByAddressMutation, useSignMessageByIdMutation } from '@ssdcoin-network/api-react';
import { Button, Card, Flex, Form, TextField, useOpenDialog, useShowError } from '@ssdcoin-network/core';
  ...
  async function handleSignById(messageToSign: string, id: string, address: string) {
    ...
    if (error) {
      ...
      if (missingNFTMatch || missingDIDMatch) {
        ...
        const entityId = toBech32m(entityPuzzleHash, missingNFTMatch ? 'nft' : 'did:ssd:');
