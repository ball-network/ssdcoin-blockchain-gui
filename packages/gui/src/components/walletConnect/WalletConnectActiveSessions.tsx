import { Flex, Loading, TooltipIcon } from '@ssdcoin-network/core';
...
const supportedCommands = WalletConnectCommands.map((item) => `ssdcoin_${item.command}`);
        ...
        {isLoading ? (
          ...
          <Flex flexDirection="column" gap={2}>
            {pair.sessions.map((session) => {
              ...
              const methods = namespaces.ssdcoin?.methods ?? [];
                ...
                <Flex flexDirection="column">
                  ...
                  {unsupportedMethods.length ? (
                    <Typography variant="body2" color="textSecondary">
                      ...
                      <TooltipIcon>
                        <Trans>
                          Your current version of the SSDCoin application may not support some commands requested by the
