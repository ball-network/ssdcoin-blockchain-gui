...
type CreateDefaultValuesParams = {
  walletType?: WalletType; // CAT or STANDARD_WALLET (SSD), indicates whether a token or CAT has a default entry
 ...
 * for creating an SSD offer, a CAT offer, or an NFT offer. SSD and CAT offers will have a default entry in
...
export default function createDefaultValues(params: CreateDefaultValuesParams | undefined = {}): OfferBuilderData {
  ...
  return {
    ...
    offered: {
      ...
      nfts,
      ssd: walletType === WalletType.STANDARD_WALLET ? [{ amount: '' }] : [],
