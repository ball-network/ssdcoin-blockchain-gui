import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@ssdcoin-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@ssdcoin-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@ssdcoin-network/api-react';
import { catToMojo, ssdcoinToMojo } from '@ssdcoin-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedSSD = useWatch({
    name: 'requested.ssd',
  });

  const offeredSSD = useWatch({
    name: 'offered.ssd',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedSSD ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('ssd', ssdcoinToMojo(amount))),
    ...(requestedTokens ?? [])
