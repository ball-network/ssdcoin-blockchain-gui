import { ConfirmDialog, Flex } from '@ssdcoin-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'SSD' && assetStatus.type === 'SSD') {
      // assetId might be either 'SSD' or 'UNKNOWN'(maybe fee) but here both are treated as just a SSD spending
