import { Color, Flex } from '@ssdcoin-network/core';
import { Offering, Requesting } from '@ssdcoin-network/icons';
...
import OfferBuilderSSDSection from './OfferBuilderSSDSection';
...
export default function OfferBuilderTradeColumn(props: OfferBuilderTradeColumnProps) {
  ...
  const showSSD = !readOnly || !!ssd.length;
  ...
  const mutedSSD = nfts.length || tokens.length;
  const mutedTokens = ssd.length || nfts.length;
  const mutedNFTs = ssd.length || tokens.length;
  ...
  return (
    <Flex flexDirection="column" gap={3}>
      ...
      >
        {showSSD && <OfferBuilderSSDSection name={`${name}.ssd`} offering={offering} muted={mutedSSD} />}
