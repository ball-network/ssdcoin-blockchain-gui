import { toBech32m, WalletType } from '@ssdcoin-network/api';
import { mojoToSSDCoin, mojoToCAT } from '@ssdcoin-network/core';
...
export default function createOfferForIdsToOfferBuilderData(
      ...
      if (asset) {
        switch (asset.walletType) {
          case WalletType.STANDARD_WALLET:
            section.ssd.push({ amount: mojoToSSDCoin(numericValue.abs()).toFixed() });
            break;
          case WalletType.CAT:
  ...
  if (fee) {
    offerBuilderData.offered.fee = [{ amount: mojoToSSDCoin(fee).toFixed() }];
  }
