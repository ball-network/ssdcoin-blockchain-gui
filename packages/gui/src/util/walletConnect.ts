...
const log = initDebug('ssdcoin-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `ssdcoin_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.ssdcoin;
    if (!requiredNamespace) {
      throw new Error('Missing required ssdcoin namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['ssdcoin:testnet', 'ssdcoin:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `ssdcoin:${instance}:${fingerprint}`);
