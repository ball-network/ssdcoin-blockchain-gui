...
export function getUserDataDir(): string {
  const ssdcoinRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(ssdcoinRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const ssdcoinRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${ssdcoinRootUserDataPath}`);
  app.setPath('userData', ssdcoinRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(ssdcoinRootUserDataPath, 'Local Storage', 'leveldb');
