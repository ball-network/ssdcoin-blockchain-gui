import { useLocalStorage } from '@ssdcoin-network/api-react';
...
export default function useGetLatestVersionFromWebsite(): UseGetLatestVersionFromWebsiteResult {
  ...
  const [latestVersionURL] = useLocalStorage<string>(
    'latestVersionURL',
    'https://download.ssdcoin.top/latest/latest.json'
  );
    ...
    ipcRenderer.invoke('fetchHtmlContent', latestVersionURL).then((obj: any) => {
      try {
        ...
        /* we don't need to handle error here, if we are unable to fetch version number
           from ssdcoin.top, we just ignore showing reminder dialog */
      }
    });
  }, [latestVersionURL]);

  const downloadUrl = downloadPath ? new URL(downloadPath, 'https://www.ssdcoin.top/').toString() : undefined;
  const releaseNotesUrl = releaseNotesPath ? new URL(releaseNotesPath, 'https://www.ssdcoin.top/').toString() : undefined;
  const blogUrl = blogPath ? new URL(blogPath, 'https://www.ssdcoin.top/').toString() : undefined;
