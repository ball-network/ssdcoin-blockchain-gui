import { useGetFeeEstimateQuery } from '@ssdcoin-network/api-react';
...
import mojoToSSDCoinLocaleString from '../../utils/mojoToSSDCoinLocaleString';
import Fee from '../Fee';
...
export enum FeeTxType {
  walletSendSSD = 'send_ssd_transaction',
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const formattedEstimates: FormattedEstimate[] = useMemo(() => {
    ...
    return estList.map((estimate: number, i: number) => {
      const formattedEstimate = mojoToSSDCoinLocaleString(estimate, locale);
  ...
  useEffect(() => {
    if (formattedEstimates && inputType === 'dropdown') {
      ...
      if (selectedTime) {
        ...
        if (estimate) {
          const ssdFee = mojoToSSDCoinLocaleString(estimate.estimate, 'en-US');
          setSelectedValue(estimate.formattedEstimate);
          setValue(name, ssdFee);
        }
      }
    }
