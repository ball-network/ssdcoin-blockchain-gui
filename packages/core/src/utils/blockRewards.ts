...
export function calculatePoolReward(height: number): BigNumber {
  if (height === 0) {
    return MOJO_PER_SSDCOIN.times(3_000_000).times(POOL_REWARD)
  }
...
export function calculateBaseFarmerReward(height: number): BigNumber {
  ...
  if (height === 0) {
    return MOJO_PER_SSDCOIN.times(3_000_000).times(FARMER_REWARD)
  }
...
export function calculateStakeReward(height: number): BigNumber {
  return MOJO_PER_SSDCOIN.times(calculateReward(height)).times(STAKE_REWARD);
}
