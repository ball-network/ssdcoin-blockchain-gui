import { WalletType, TransactionType, TransactionTypeFilterMode, toBech32m } from '@ssdcoin-network/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate, location) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          <strong>
            <FormatLargeNumber
              value={[WalletType.CAT, WalletType.CRCAT].includes(type) ? mojoToCAT(row.amount) : mojoToSSDCoin(row.amount)}
            />
          </strong>
          &nbsp;
          {metadata.unit}
...

  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToSSDCoin(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
