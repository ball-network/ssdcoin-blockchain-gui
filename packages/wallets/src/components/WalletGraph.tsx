import { TransactionType, WalletType } from '@ssdcoin-network/api';
import type { Transaction } from '@ssdcoin-network/api';
import { useGetWalletBalanceQuery } from '@ssdcoin-network/api-react';
import { Color, mojoToSSDCoin, mojoToCAT, blockHeightToTimestamp } from '@ssdcoin-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToSSDCoin(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: (walletType === WalletType.CAT ? mojoToCAT(balance) : mojoToSSDCoin(balance)).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToSSDCoin(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: walletType === WalletType.CAT ? mojoToCAT(start) : mojoToSSDCoin(start).toString(), // bignumber is not supported by react
    });
  });
