  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(
        0,
        ([WalletType.CAT, WalletType.CRCAT].includes(walletType) ? mojoToCAT(start) : mojoToSSDCoin(start)).toNumber()
      ), // max 21,000,000 safe to number
      tooltip: ([WalletType.CAT, WalletType.CRCAT].includes(walletType)
        ? mojoToCAT(balance)
        : mojoToSSDCoin(balance)
  ...
  data.forEach((item) => {
    ...
    points.push({
      ...
      tooltip: [WalletType.CAT, WalletType.CRCAT].includes(walletType)
        ? mojoToCAT(start)
        : mojoToSSDCoin(start).toString(), // bignumber is not supported by react
    });
  });
