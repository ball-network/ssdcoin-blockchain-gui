import {TransactionType, TransactionTypeFilterMode, StakeValue, toBech32m} from '@ssdcoin-network/api';
import {
  AddressBookContext,
  Card,
  CopyToClipboard,
...
import type { Row } from '@ssdcoin-network/core';
...
const getCols = (isStakeFarm: boolean) => [
  {
    field: (row: Row, metadata: any) => (
        <>
          <strong>
            <FormatLargeNumber value={mojoToSSDCoin(row.amount)} />
          </strong>
          &nbsp;
          {metadata.feeUnit}
        </>
...
export default function StakeHistory(props: Props) {
  ...
  const {
    ...
    typeFilter: {
      ...
      values: [
        isStakeFarm ? TransactionType.INCOMING_STAKE_FARM_RECEIVE : TransactionType.INCOMING_STAKE_FARM_RECEIVE
      ],
    },
  });
        ...
        caption={
          !transactions?.length && (
            <Typography variant="body2" align="center">
              {isStakeFarm ? <Trans>No previous stake farm transactions</Trans> : <Trans>No previous stake farm transactions</Trans>}
            </Typography>
          )
        }
