import { SyncingStatus, toBech32m, WalletType } from '@ssdcoin-network/api';
import { useSpendCATMutation, useFarmBlockMutation } from '@ssdcoin-network/api-react';
import {
  AdvancedOptions,
  Button,
  EstimatedFee,
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(t`Cannot send ssdcoin to coloured address. Please enter a ssdcoin address.`);
    }

    if (address.includes('ssdcoin_addr') || address.includes('colour_desc')) {
    ...
    if (address.slice(0, 15) === 'ssdcoin_addr://') {
    ...
    const feeValue = ssdcoinToMojo(fee);
